...
export default function KundeEditV2Page() {
    ...
    async function load() {
        ...
        var kundeResult = await supabase
          .from('kunden')
          .select('*')
          .eq('id', id)
        ...
        if (!kundeResult.data) { setLaden(false); return; }
        var k = kundeResult.data;
        setForm({
            name: k.name || '',
            firma: k.firma || k.firmenname || '',
            telefon: k.telefon || '',
            email: k.email || '',
            adresse: k.adresse || '',
            notizen: k.notizen || '',
    ...
    async function handleSubmit(e) {
        ...
        var updateResult = await supabase.from('kunden').update({
            name: form.name,
    ...
    async function handleDelete() {
        setIsDeleting(true);
        var deleteResult = await supabase.from('kunden').delete().eq('id', id).eq('company_id', companyId);
        if (deleteResult.error) {
            setApiErr('Fehler beim Löschen. Bitte erneut versuchen.');
            setIsDeleting(false);
            return;
        }
        router.push('/dashboard-v2/kunden');
    }

    if (laden) {
        return (
            <Page>
              <Page.Header><Page.Title>Kunde bearbeiten</Page.Title></Page.Header>
              <Page.Content>
    ...
    if (!form) {
        return (
            <Page>
              <Page.Header><Page.Title>Kunde bearbeiten</Page.Title></Page.Header>
              <Page.Content>
                <div className="py-12 text-center text-sm text-red-500">Kunde nicht gefunden.</div>
              </Page.Content>
            </Page>
        );
    }
    ...
    return (
        <Page>
          <Page.Header>
            <Page.Title>Kunde bearbeiten</Page.Title>
            <Page.Description>Kundendaten bearbeiten und speichern</Page.Description>
          </Page.Header>
          <Page.Content>
            {success && (
            ...
            {loeschenBestaetigen && (
              <div className="mb-4 rounded-lg bg-red-50 border border-red-300 px-4 py-4">
                <p className="text-sm font-medium text-red-800 mb-3">
                  Kunden wirklich löschen? Diese Aktion kann nicht rückgängig gemacht werden.
                </p>
                <div className="flex gap-3">
                  <Button variant="danger" disabled={isDeleting} onClick={handleDelete}>
                    {isDeleting ? 'Löschen...' : 'Ja, löschen'}
                  </Button>
                  <Button variant="secondary" onClick={function() { setLoeschenBestaetigen(false); }}>
                    Abbrechen
                  </Button>
                </div>
              </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex items-center justify-between border-t border-gray-100 pt-4">
                    <Button
                      type="button"
                      variant="danger"
                      onClick={function() { setLoeschenBestaetigen(true); setApiErr(''); }}
                    >
                      Kunde löschen
                    </Button>

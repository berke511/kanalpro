...
var LEISTUNGEN = [
  ...
  'Close-Fit-Lining','Tight-Fit-Lining','Sprøhliner','Beschichtung','Innenbeschichtung',
  ...
  'Abscheiderentleerung','Kanalzustandsbewertung','Sanierungs6onzept','Sanierungsplanung',
  ...
  'Havariesanierung','Rohrbruchbeseitigung','Wasserschadenservice','Freispølen von Leitungen',
...
export default function AngebotNeuV2Page() {
  ...
  async function ladeDaten() {
    ...
    setCompanyId(cid);
    var cr = await supabase.from('companies').select('standard_steuersatz').eq('id', cid).single();
    if (cr.data && cr.data.standard_steuersatz) {
      setForm(function(f) { return Object.assign({}, f, { steuersatz: cr.data.standard_steuersatz }); });
    }
    var kr = await supabase.from('kunden').select('id, name').eq('company_id', cid).order('name');
    setKunden(kr.data ?? []);
    setLaden(false);
  }
  ...
  async function handleSpeichern(e) {
    ...
    try {
      var r = await supabase.auth.getUser();
      var user = r.data.user;
      var cr = await supabase.from('angebote').select('*', { count: 'exact', head: true }).eq('company_id', companyId);
      var year = new Date().getFullYear();
      var num = String((cr.count ?? 0) + 1).padStart(3, '0');
      var nr = 'AN-' + year + '-' + num;
      var ins = await supabase.from('angebote').insert({
        user_id: user.id,
      ...
      setTimeout(function() { router.push('/dashboard-v2/angebote'); }, 800);
    } catch (err) {
  ...
  if (laden) return (
    <Page>
      <Page.Header><Page.Title>Angebot erstellen</Page.Title></Page.Header>
  ...
  return (
    <Page>
      <Page.Header>
        <Page.Title>Neues Angebot</Page.Title>
        <Page.Description>Angebot erstellen und Positionen erfassen</Page.Description>
      </Page.Header>
      <Page.Content>
        <form onSubmit={handleSpeichern} className="space-y-4 max-w-3xl">
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button type="submit" variant="primary" disabled={speichern}>
              {speichern ? 'Wird gespeichert...' : 'Angebot speichern'}
            </Button>

...
// Firmendaten aus company-Parameter extrahieren (FIX: logoUrl war undefiniert)
...
doc.setFontSize(22); doc.setFont('helvetica', 'bold'); doc.text(company?.name || 'KanalPro', 15, 18);
...
// Absender + Empfänger (FIX: war hardcoded 'Ihr Unternehmen · Musterstraße 1 · 40000 Düsseldorf')
doc.setFontSize(8); doc.setTextColor(...grau);
const absenderTeile = [
  company?.name,
  company?.strasse,
  (company?.plz || company?.ort) ? `${company?.plz ?? ''} ${company?.ort ?? ''}`.trim() : null,
].filter(Boolean);
if (absenderTeile.length > 0) {
  doc.text(absenderTeile.join(' · '), 15, 45);
}

...
function SchnellKarte({ tabId, router, active, onSelect }) {
  const m = MODULE[tabId];
  if (!m) return null;
  return (
    <button
    ...
    >
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${m.iconColor}`}>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
          strokeWidth={1.5} stroke="currentColor" className={`w-5 h-5 ${m.textColor}`} aria-hidden="true">
          <path strokeLinecap="round" strokeLinejoin="round" d={m.icon} />
        </svg>
      </div>
      <div className="min-w-0">
        <p className={`text-sm font-medium ${active ? 'text-blue-700' : 'text-gray-900'}`}>{m.titel}</p>
        <p className="text-xs text-gray-400 truncate">{m.beschreibung.split('.')[0]}.</p>
      </div>
    </button>
  );
}
  ...
  return (
    <div className="max-w-screen-2xl mx-auto space-y-5 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Linke Spalte: Tab-Navigation + Aktives Modul */}
        ...
        {/* Rechte Spalte: Schnellzugriff (Desktop only) */}
        <div className="hidden lg:flex flex-col gap-3">
          <p className="text-sm font-semibold text-gray-700">Alle Module</p>

...
const LEER = {
  typ: '', prioritaet: 'Mittel', kunden_id: '', ansprechpartner: '',
  telefon: '', email: '', strasse: '', plz: '', ort: '', zusatz: '',
  zugang: '', einsatzdatum: '', startzeit: '', dauer: '',
  mitarbeiter_id: '', fahrzeug_id: '', beschreibung: '', notizen: '',
};
...
function Skeleton() {
  return (
    <div className="space-y-6 animate-pulse">
      {[1,2,3].map(i => <div key={i} className="bg-white rounded-2xl border border-gray-100 h-40" />)}
    </div>
  );
}
...
export default function AuftragErstellen() {
  ...
  useEffect(() => {
    async function laden() {
      try {
        ...
        setCompanyId(cid);
        const [{ data: k }, { data: m }, { data: f }] = await Promise.all([
  ...
  async function speichern(status = 'Neu') {
    if (!validieren()) { window.scrollTo({ top: 0, behavior: 'smooth' }); return; }
    setSaving(true); setApiErr('');
    try {
      const payload = {
        company_id: companyId, nummer: genNummer(), typ: form.typ,
        prioritaet: form.prioritaet, status,
  ...
  const kundenName = (k) => k.kundentyp === 'firma' && k.firmenname ? k.firmenname : k.name;
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Fülle alle Pflichtfelder aus, um den Auftrag zu erstellen.</p>
        </div>
        <button onClick={() => router.push('/dashboard/auftraege')} disabled={saving}
            ...
            <div className="flex gap-2 pt-0.5">
              {PRIORITAETEN.map(p => (
                <button key={p} type="button" onClick={() => set('prioritaet', p)}
                  className={`flex-1 py-2 rounded-xl text-xs font-semibold border transition ${form.prioritaet === p ? PRIO_FARBEN[p] + ' ring-2 ring-offset-1 ring-blue-400' : 'border-gray-200 text-gray-400 hover:border-gray-300'}`}>
                  {p}
                </button>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field error={fehler.kunden_id}>
            ...
            <select id="kunden_id" value={form.kunden_id} onChange={e => onKundeWaehlen(e.target.value)}
              ...
              <option value="">— Bitte wählen —</option>
              {kunden.map(k => <option key={k.id} value={k.id}>{kundenName(k)}</option>)}
            </select>
          </Field>
          <Field>
            <Label htmlFor="ansprechpartner">Ansprechpartner</Label>
            <input id="ansprechpartner" type="text" value={form.ansprechpartner} onChange={e => set('ansprechpartner', e.target.value)} placeholder="Name des Ansprechpartners" className={inp('border-gray-200')} />
          </Field>
          <Field>
            <Label htmlFor="telefon">Telefon</Label>
            <input id="telefon" type="tel" value={form.telefon} onChange={e => set('telefon', e.target.value)} placeholder="+49 30 1234567" className={inp('border-gray-200')} />
          </Field>
          <Field>
            <Label htmlFor="email">E-Mail</Label>
            <input id="email" type="email" value={form.email} onChange={e => set('email', e.target.value)} placeholder="kontakt@kunde.de" className={inp('border-gray-200')} />
          </Field>
        </div>
      </SectionCard>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            <Field>
              ...
              <input id="strasse" type="text" value={form.strasse} onChange={e => set('strasse', e.target.value)} placeholder="Musterstraße 12" className={inp('border-gray-200')} />
            </Field>
          </div>
          <Field>
            <Label htmlFor="plz">PLZ</Label>
            <input id="plz" type="text" value={form.plz} onChange={e => set('plz', e.target.value)} placeholder="12345" maxLength={5} className={inp('border-gray-200')} />
          </Field>
          <Field>
            <Label htmlFor="ort">Ort</Label>
            <input id="ort" type="text" value={form.ort} onChange={e => set('ort', e.target.value)} placeholder="Berlin" className={inp('border-gray-200')} />
          </Field>
          <Field>
            <Label htmlFor="zusatz">Einheit / Stockwerk</Label>
            <input id="zusatz" type="text" value={form.zusatz} onChange={e => set('zusatz', e.target.value)} placeholder="z. B. EG links, Keller" className={inp('border-gray-200')} />
          </Field>
          <Field>
            <Label htmlFor="zugang">Zugangsinformationen</Label>
            <input id="zugang" type="text" value={form.zugang} onChange={e => set('zugang', e.target.value)} placeholder="z. B. Schlüssel im Büro" className={inp('border-gray-200')} />
          </Field>
        </div>
      </SectionCard>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Field error={fehler.einsatzdatum}>
            ...
            <input id="einsatzdatum" type="date" value={form.einsatzdatum} onChange={e => set('einsatzdatum', e.target.value)} className={inp(fehler.einsatzdatum ? 'border-red-300' : 'border-gray-200')} />
          </Field>
          <Field>
            <Label htmlFor="startzeit">Startzeit</Label>
            <input id="startzeit" type="time" value={form.startzeit} onChange={e => set('startzeit', e.target.value)} className={inp('border-gray-200')} />
          </Field>
          <Field>
            <Label htmlFor="dauer">Geschätzte Dauer (Std.)</Label>
            <input id="dauer" type="number" min="0.5" max="24" step="0.5" value={form.dauer} onChange={e => set('dauer', e.target.value)} placeholder="z. B. 2" className={inp('border-gray-200')} />
          </Field>
        </div>
      </SectionCard>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field>
            ...
            <select id="mitarbeiter_id" value={form.mitarbeiter_id} onChange={e => set('mitarbeiter_id', e.target.value)} className={inp('border-gray-200')}>
          ...
          <Field>
            ...
            <select id="fahrzeug_id" value={form.fahrzeug_id} onChange={e => set('fahrzeug_id', e.target.value)} className={inp('border-gray-200')}>
              <option value="">— Noch nicht zugewiesen —</option>
              {fahrzeuge.map(f => <option key={f.id} value={f.id}>{f.kennzeichen}{f.marke ? ` · ${f.marke}` : ''}{f.modell ? ` ${f.modell}` : ''}</option>)}
            </select>
          </Field>
        </div>
      </SectionCard>
      ...
      >
        <div className="space-y-4">
          <Field error={fehler.beschreibung}>
            ...
            <textarea id="beschreibung" rows={4} value={form.beschreibung} onChange={e => set('beschreibung', e.target.value)} placeholder="Beschreibe das Problem oder den Auftrag so detailliert wie möglich …" className={inp(fehler.beschreibung ? 'border-red-300 resize-none' : 'border-gray-200 resize-none')} />
          </Field>
          <Field>
            <Label htmlFor="notizen">Interne Notizen</Label>
            <textarea id="notizen" rows={3} value={form.notizen} onChange={e => set('notizen', e.target.value)} placeholder="Nur intern sichtbar – z. B. Besonderheiten, Vorgeschichte …" className={inp('border-gray-200 resize-none')} />
          </Field>
        </div>
      </SectionCard>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 px-5 py-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        <button type="button" onClick={() => router.push('/dashboard/auftraege')} disabled={saving}
        ...
        <button type="button" onClick={() => speichern('Neu')} disabled={saving}
          ...
          {saving ? <Svg d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" cls="w-4 h-4 animate-spin" /> : <Svg d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />}
          Als Entwurf speichern
        </button>
        <button type="button" onClick={() => speichern('Geplant')} disabled={saving}
          ...
          {saving ? <Svg d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" cls="w-4 h-4 animate-spin" /> : <Svg d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />}
          Auftrag erstellen
        </button>
      </div>

    ...
    <Karte>
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {gespeichert && <span className="text-xs text-green-600 dark:text-green-400 font-medium">Unterschrift gespeichert</span>}
        </div>
  ...
  return (
    <Karte>
      ...
      <div className="px-5 py-4 space-y-2">
        {checks.map(c => (
          <label key={c.key} className="flex items-center gap-3 cursor-pointer select-none p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition" style={{ minHeight: '44px' }}>
            ...
            <span className={`text-sm font-medium flex-1 ${form[c.key] ? 'text-gray-700 dark:text-gray-200' : 'text-gray-400 dark:text-gray-500'}`}>{c.label}</span>
...
/* ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
   SEKTION 10 â ABSCHLUSS + AKTIONEN
ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ */

function AbschlussSektion({ dok, material, fotos, onAbschliessen, onRechnung, onNaechsterEinsatz, saving, fehler }) {
  const checks = [
    { label: 'Status gesetzt',           ok: !!dok?.einsatz_status },
    { label: 'TÃ¤tigkeiten dokumentiert', ok: !!dok?.durchgefuehrte_arbeiten },
    { label: 'Material erfasst',         ok: dok?.kein_material_verwendet || material.length > 0 },
  ...
  return (
    <Karte className={abgeschlossen ? 'border-green-200 dark:border-green-800' : ''}>
      <KarteHeader
        ...
        {abgeschlossen ? (
          <>
            <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-100 dark:border-green-800 text-center">
              ...
              <p className="font-semibold text-green-700 dark:text-green-300">Einsatzbericht abgeschlossen</p>
              {dok?.dokumentiert_at && (
                <p className="text-xs text-green-500 mt-1">Abgeschlossen um {fmtZeit(dok.dokumentiert_at)}</p>
              )}
            </div>
            <div className="flex flex-col gap-3">
              <button onClick={onRechnung} style={{ minHeight: '44px' }}
                className="w-full flex items-center justify-center gap-2 py-3 bg-blue-600 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition">
              ...
              <button onClick={onNaechsterEinsatz} style={{ minHeight: '44px' }}
                className="w-full flex items-center justify-center gap-2 py-3 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-xl text-sm font-semibold hover:bg-gray-200 dark:hover:bg-gray-600 transition">
                <Svg d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" cls="w-4 h-4" />
          ...
          <>
            {!allOk && (
              <div className="space-y-1.5 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-100 dark:border-amber-800">
                <p className="text-xs font-semibold text-amber-700 dark:text-amber-300 uppercase tracking-wide mb-2">Noch ausstehend:</p>
                {checks.filter(c => !c.ok).map(c => (
                  <div key={c.label} className="flex items-center gap-2 text-sm text-amber-600 dark:text-amber-400">
            ...
            <button onClick={onAbschliessen} disabled={saving || !allOk} style={{ minHeight: '44px' }}
...
function EinsatzberichtPageInner() {
  ...
  const [zeitForm, setZeitForm] = useState({
    arbeit_start: '', pause_beginn: '', pause_ende: '', arbeit_ende: '',
  });
  const [unterschrForm, setUnterschrForm] = useState({
    kundenname: '', unterschrift_base64: null, unterschrift_at: null,
  });
  ...
  const ladeDaten = useCallback(async () => {
    ...
    try {
      ...
      if (!erlaubt.includes(member.rolle)) { setZustand('forbidden'); return; }
  ...
  async function saveZeiten() {
    setSaving(true);
    try {
      const pauseMin = timeDiffMin(zeitForm.pause_beginn, zeitForm.pause_ende);
      await updateDok({
        arbeit_start:  zeitForm.arbeit_start || null,
        arbeit_ende:   zeitForm.arbeit_ende  || null,
  ...
  function handleRechnungVorbereiten() {
    router.push(`/dashboard/rechnungen/new?auftrag_id=${auftragId}`);
  }

  async function handleNaechsterEinsatz() {
    try {
      const heute = auftrag?.datum ?? new Date().toISOString().slice(0, 10);
      const { data } = await supabase
        ...
        .eq('datum', heute)
        .neq('id', auftragId)
        .not('status', 'in', '("Abgeschlossen","dokumentiert")')
        .order('uhrzeit', { ascending: true })
        .limit(1)
        .maybeSingle();

      if (data?.id) {
        router.push(`/dashboard/auftraege/${data.id}`);
      } else {
        setGlobalErr('Kein weiterer Einsatz fÃ¼r heute gefunden.');
        setTimeout(() => setGlobalErr(''), 4000);
      }
    } catch (e) {
      setGlobalErr(e.message ?? 'Fehler beim Suchen des nÃ¤chsten Einsatzes');
    }
  }
  ...
  if (zustand === 'forbidden') return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-4">
      <div className="text-center">
        ...
        <h2 className="font-semibold text-gray-700 dark:text-gray-200 mb-1">Kein Zugriff</h2>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 sticky top-0 z-30">
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={() => router.push(`/dashboard/auftraege/${auftragId}`)} style={{ minHeight: '44px', minWidth: '44px' }}
            className="p-2 rounded-xl text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 hover:text-gray-600 transition">
            <Svg d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" cls="w-5 h-5" />
          </button>
          <div className="flex-1 min-w-0">
            <h1 className="text-base font-bold text-gray-900 dark:text-white truncate">Einsatz & Dokumentation</h1>
            <p className="text-xs text-gray-400 truncate">
              {auftrag?.typ ?? auftrag?.titel ?? 'Auftrag'} Â· #{auftrag?.auftragsnummer ?? auftragId?.slice(0, 8)}
            </p>
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 py-5 space-y-4">
        ...
        <FotosSektion
          ...
          onFotoDeleted={id => setFotop(p => p.filter(f => f.id !== id))}
        />
        ...
        <AbschlussSektion
          ...
          onRechnung={handleRechnungVorbereiten}
          onNaechsterEinsatz={handleNaechsterEinsatz}
          saving={saving}
          fehler={abschlFehler}
        />

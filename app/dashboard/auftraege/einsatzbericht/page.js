...
function TaetigkeitenSektion({ form, onChange, onSave, saving, gespeichert }) {
  const felder = [
    ...
    { key: 'festgestellter_schaden',  label: 'Festgestellter Schaden',   placeholder: 'Sch��en, M�ngel…',    rows: 2 },
    { key: 'ursache',                 label: 'Ursache',                   placeholder: 'Ursache des Problems…', rows: 2 },
    { key: 'massnahmen',              label: 'Ma�nahmen',                 placeholder: 'Empfohlene Ma�nahmen…',    rows: 2 },
    { key: 'interne_notiz',           label: 'Interne Notiz',             placeholder: 'Interne Anmerkungen…',    rows: 2 },
  ];
  return (
    <Karte>
      <KarteHeader
        ...
        action={
          <button onClick={onSave} disabled={saving}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600 text-white rounded-xl text-xs font-semibold hover:bg-blue-700 transition disabled:opacity-50">
            {saving ? <Svg d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" cls="w-3.5 h-3.5 animate-spin" /> : <Svg d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />}
            Speichern
          </button>
        }
      />
      <div className="px-5 py-4 space-y-4">
        {gespeichert && <Alert type="success" onClose={gespeichert.onClose}>Dokumentation gespeichert!</Alert>}
        {felder.map(f => (
          <div key={f.key}>
            <Label>{f.label}</Label>
            <textarea
              ...
              className={inp(f.required && !form[f.key]?.trim())}
            />
          </div>
        ))}
      </div>
...
function MaterialSektion({ material, onAdd, onDelete, keinMaterial, onKeinMaterial, saving }) {
  const [new, setNeu]     = useState({ bezeichnung: '', menge: '1', einheit: 'Stk.', bemerkung: '' });
  ...
  return (
    <Karte>
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        <div className="flex items-center gap-3">
          <BtnPrimary onClick={onSave} loading={saving} disabled={!hatZeich || !form.kundelname}>
            <Svg d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" cls="w-4 h-4" />
            Speichern
          </BtnPrimary>

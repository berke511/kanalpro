  ...
  const ladeDaten = useCallback(async () => {
    try {
      ...
      const { data: member } = await supabase
        ...
        .select('company_id, role, mitarbeiter:mitarbeiter_id(vorname, nachname)')
        .eq('user_id', user.id)
        .eq('is_active', true)
        .maybeSingle();
      ...
      {auftragTab === 'abschluss' && (
        <div className="max-w-2xl space-y-5">
          ...
          {(() => {
            ...
            const erfuelltCount = checks.filter(c => c.ok).length;
                ...
                <div className="px-5 py-5 space-y-3">
                  {checks.map((c, i) => (
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <span className={`text-sm ${c.ok ? 'text-gray-700 font-medium' : 'text-gray-400'}`}>
                        {c.label}
                      </span>
                    </div>
                  ))}

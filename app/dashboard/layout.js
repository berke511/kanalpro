...
export default function DashboardLayout({ children }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      {/* ── Desktop Top-Bar (nur md+) ────────────────────────────── */}
      <header className="hidden md:flex items-center gap-4 px-6 h-14 bg-white border-b border-gray-100 sticky top-0 z-30 shrink-0">
        <GlobalSearch />
        <span className="ml-auto text-xs text-gray-400 truncate max-w-[220px] select-none">
          {user.email}
        </span>
      </header>

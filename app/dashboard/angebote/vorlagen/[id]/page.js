...
export default function VorlageBearbeiten() {
  ...
  useEffect(() => {
    async function load() {
      const { data: vl } = await supabase
        ...
        .eq('id', id)
        .single();
      if (!vl) { router.push('/dashboard/angebote/vorlagen'); return; }
      setName(vl.name ?? '');
      setSteuersatz(vl.steuersatz ?? 19);
      setNotizen(vl.notizen ?? '');
      setPositionen(
  ...
  const mwst   = netto * (Number(steuersatz) / 100);
  ...
  async function onSubmit(e) {
    ...
    if (!name.trim()) return;
    setSpeichern(true);
    await supabase.from('angebot_vorlagen').update({
      name:       name.trim(),
      steuersatz: Number(steuersatz),
      positionen,
      notizen:    notizen || null,
    }).eq('id', id);
    setSpeichern(false);
    router.push('/dashboard/angebote/vorlagen');
  }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 pb-10">
      ...
      <div className="flex items-center justify-between">
        <div>
          <Link href="/dashboard/angebote/vorlagen" className="text-xs text-gray-400 hover:text-gray-600 transition">
            ← Zurøck zu Vorlagen
          </Link>
          <h1 className="text-xl font-bold text-gray-900 mt-1">Vorlage bearbeiten</h1>
        </div>
        <button
          type="button"
      ...
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-3 bg-gray-50 border-b border-gray-200">
            <h2 className="text-sm font-semibold text-gray-700">Vorlagenname</h2>
          </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="px-5 py-3 bg-gray-50 border-t border-gray-200 flex justify-end">
            <div className="min-w-[220px] space-y-1.5 text-sm">
              ...
              <div className="flex justify-between text-gray-500">
                <span>MwSt. {steuersatz} %</span><span className="tabular-nums">{fmt(mwst)}</span>
              </div>
      ...
      {deleteConfirm && (
        ...
        >
          ...
          >
            <h3 className="text-base font-semibold text-gray-900 mb-1">Vorlage löschen?</h3>

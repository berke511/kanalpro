...
useEffect(() => {
async function load() {
// FIX: Firmendaten aus companies-Tabelle via company_members (statt einstellungen)
const { data: { user } } = await supabase.auth.getUser();
const [{ data: rech }, { data: memberData }] = await Promise.all([
...
supabase.from('company_members')
  .select('companies(name, strasse, plz, ort, telefon, email, ust_id, steuernummer, iban, bic, bank, logo_url)')
  .eq('user_id', user.id)
  .single(),
]);
setRechnungen(rech ?? []);
setFirma(memberData?.companies ?? null);
setLaden(false);
}
load();
}, []);
...
const doc = new jsPDF();
// FIX: firma.name statt firma.firmaname; alle verfügbaren Felder ausgeben
const firmaname = firma?.name ?? 'Unbekannt';
const adresseParts = [
  firma?.strasse,
  (firma?.plz || firma?.ort) ? `${firma?.plz ?? ''} ${firma?.ort ?? ''}`.trim() : null,
].filter(Boolean);
...
doc.text(`${firmaname}${adresse ? ' · ' + adresse : ''}`, 17, 29);
...
if (kontaktTeile.length > 0) {
  doc.text(kontaktTeile.join(' · '), 17, headerY);
  headerY += 6;
}
const steuerTeile = [
  firma?.ust_id ? `USt-ID: ${firma.ust_id}` : null,
  firma?.steuernummer ? `StNr: ${firma.steuernummer}` : null,
].filter(Boolean);
if (steuerTeile.length > 0) {
  doc.text(steuerTeile.join(' · '), 17, headerY);
  headerY += 6;
}
doc.text(`Zeitraum: ${zeitraum} · Exportdatum: ${new Date().toLocaleDateString('de-DE')}`, 17, headerY);
headerY += 6;
doc.setTextColor(0);

doc.setDrawColor(220, 220, 220);
doc.line(17, headerY, 195, headerY);
...
// FIX: firma.name statt firma.firmaname
...
doc.setFont('helvetica', 'normal');
doc.setTextColor(100);
if (firmaname) doc.text(firmaname, 17, 30);
if (adresseParts.length > 0) doc.text(adresseParts.join(', '), 17, 36);
const kontaktTeile2 = [
  firma?.telefon ? `Tel: ${firma.telefon}` : null,
  firma?.email ? `E-Mail: ${firma.email}` : null,
].filter(Boolean);
if (kontaktTeile2.length > 0) doc.text(kontaktTeile2.join(' · '), 17, 42);
const steuerTeile2 = [
  firma?.ust_id ? `USt-ID: ${firma.ust_id}` : null,
  firma?.steuernummer ? `StNr: ${firma.steuernummer}` : null,
].filter(Boolean);
if (steuerTeile2.length > 0) doc.text(steuerTeile2.join(' · '), 17, 48);
doc.setTextColor(0);

const metaY = steuerTeile2.length > 0 ? 58 : kontaktTeile2.length > 0 ? 52 : adresseParts.length > 0 ? 46 : 40;
doc.text(`Datum: ${fmtDate(r.datum)}`, 17, metaY);
doc.text(`Fällig am: ${fmtDate(r.faellig_am)}`, 17, metaY + 7);
doc.text(`Kunde: ${r.kunden?.name ?? '—'}`, 17, metaY + 14);
doc.text(`Status: ${r.status ?? '—'}`, 17, metaY + 21);

doc.setDrawColor(220, 220, 220);
const lineY = metaY + 26;
doc.line(17, lineY, 195, lineY);
...
// Bankverbindung in Fußeile (FIX: IBAN/BIC/Bank aus companies)
const bankTeile = [
...
if (bankTeile.length > 0) {
  ...
  doc.text(bankTeile.join(' · '), 17, 282);
}
}
